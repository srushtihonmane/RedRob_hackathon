# Claude Code + Fable 5 — single autonomous build prompt

**Why Fable 5 fits:** 1M-token context, adaptive thinking, memory/compaction, and it's
tuned for long-horizon agentic work that improves via its own notes — so it can hold the
whole `Architecture/` in view and run the feature loop across fewer, longer sessions.

**Setup:**
1. Open a terminal and `cd D:\Hackathon_2july` (launch from the project root — the prompt uses relative paths).
2. Select Fable 5: run `claude`, then `/model` and pick **claude-fable-5** (or start with `claude --model claude-fable-5`).
3. Pre-approve tools so the loop isn't interrupted (allow Bash/Edit/Write when first prompted).
4. Paste the block below as your first message.
5. For fully unattended multi-window runs, drive the same block with the
   [`autonomous-coding` quickstart](https://github.com/anthropics/claude-quickstarts/tree/main/autonomous-coding).

---

```
You are Claude Fable 5, building, validating, and iterating an end-to-end candidate-ranking
system AUTONOMOUSLY until it is competition-ready. Work feature-by-feature, self-verify with
real tests, and do not stop until every feature passes and a valid submission.csv is
reproducibly produced within budget.

FABLE 5 OPERATING NOTES (use your strengths):
- Use your LARGE CONTEXT: at the start of a session load overview.md + contracts.md +
  runtime.md fully and keep them in view; load the specific GoalN.md for the feature you're
  building. You don't need to trim aggressively, but still write claude-progress.txt each
  session so a fresh context window (or the quickstart re-invocation) can resume cleanly.
- Use ADAPTIVE THINKING (extended reasoning) on the hard design points — Goal 4 scoring/
  fusion/calibration (modules 4A-4E), Goal 5 trap logic, and any tie/interaction edge case.
  Think before you code these; don't one-shot them.
- Use your NOTES/MEMORY: keep a running claude-progress.txt of decisions, open risks, and
  measured timings, and improve it as you learn. Treat it as the source of truth a later
  session reads first.
- Prefer making steady, verified progress over speed. One feature, fully verified, per step.
...
- Architecture/GoalN.md      <- the detailed design for the goal you implement this session (Goal4.md
                                carries an internal 4A-4E module decomposition).
...
*Method: [Effective harnesses for long-running agents — Anthropic, Nov 26 2025](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents).
Model: [Claude Fable 5 — Anthropic, Jun 9 2026](https://www.anthropic.com/news/claude-fable-5-mythos-5).*
